...
df=pd.read_csv('../Data/CycleJulyWeek.csv')
...
# Adjust time to epoch
df['StartDate'] = (pd.to_datetime(df['Start Date']).astype('int64')//1e9).astype(int)
df['EndDate'] = (pd.to_datetime(df['End Date']).astype('int64')//1e9).astype(int)

# Add information to location for geolocating
...
# Export analysis csv
df.to_csv('../Data/JulyCycleHireAnalysis.csv')

fromtime = '04/07/2019 00:00'
...
df.to_csv('../Data/JulyCycleHire.csv')